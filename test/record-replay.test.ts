...
import * as HttpRecorderInternal from "../src/internal"
    ...
    const exit = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        return yield* Effect.exit(
          socket.runRaw(() => {}, {
            onOpen: write(new Socket.CloseEvent(1000)),
          }),
        )
      }).pipe(
        Effect.scoped,
    ...
    try {
      ...
      const responses = await Effect.runPromise(
        Effect.all([request("first"), request("second")], {
          concurrency: "unbounded",
        }).pipe(Effect.provide(HttpRecorder.http("concurrent-order", { directory }))),
      )
    ...
    const exit = await Effect.runPromise(
      Effect.exit(
        post(url, { ok: true }).pipe(
          Effect.provide(
            HttpRecorderInternal.cassetteLayer("unsafe-record", {
              directory,
              mode: "record",
            }),
          ),
